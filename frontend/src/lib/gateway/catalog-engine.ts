...
export class CatalogGatewayEngine {
  ...
  static async processQuery(
    ...
    const { data: merchants } = await supabase
      ...
      .limit(1);
    const merchant_id =
      merchants && merchants.length > 0 ? merchants[0].id : "m_demo_101";
    ...
    if (activeSessionId) {
      await MerchantAuditService.logA2ARequest(
        supabase,
        merchant_id,
        activeSessionId,
        cleanMessage,
    ...
    const isGreeting =
      /^(hi|hello|hey|greetings|good morning|good evening|good afternoon|who are you|help|thanks|thank you|okay|ok|yes|no)$/i.test(
        lower,
      ) || lower.length <= 3;

    // Parse max budget from prompt if present (e.g. "under 5000", "under ₹5,000", "below 60000")
    let maxBudget: number | null = null;
    const budgetMatch = cleanMessage.match(
      /(?:under|below|budget|less than|<|₹|\s)\s*₹?\s*(\d+[\d,]*)/i,
    );
    ...
    if (!isGreeting) {
      ...
      if (matchedProducts.length === 0) {
        if (maxBudget !== null) {
          matchedProducts = productsList
            .filter((p) => Number(p.price) <= maxBudget)
            .slice(0, 3);
        }
        if (matchedProducts.length === 0) {
    ...
    const catalogContextStr = productsList
      .map(
        (p) =>
          `- ${p.name} (Category: ${p.category}, Price: ₹${p.price}, SKU: ${p.sku})`,
      )
      .join("\n");
